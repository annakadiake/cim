...
class SecretaryDashboardView(BaseDashboardView):
    def get(self, request):
        if not (request.user.is_secretary or request.user.is_admin or request.user.is_superuser):
            return Response(
        ...
        # Patients ce mois
        patients_this_month = Patient.objects.filter(created_at__date__gte=start_of_month).count()
...
class DoctorDashboardView(BaseDashboardView):
    def get(self, request):
        if not (request.user.is_doctor or request.user.is_admin or request.user.is_superuser):
            return Response(
                {"detail": "Accès non autorisé."},
                status=status.HTTP_403_FORBIDDEN
            )
        ...
        # Rapports ce mois
        reports_this_month = PatientReport.objects.filter(created_at__date__gte=start_of_month).count()
        
        # Patients ce mois
        patients_this_month = Patient.objects.filter(created_at__date__gte=start_of_month).count()
        
        # Types d'examens actifs
        total_exam_types = ExamType.objects.filter(is_active=True).count()
...
class AccountantDashboardView(BaseDashboardView):
    def get(self, request):
        if not (request.user.is_accountant or request.user.is_admin or request.user.is_superuser):
            return Response(
                {"detail": "Accès non autorisé."},
                status=status.HTTP_403_FORBIDDEN
            )
